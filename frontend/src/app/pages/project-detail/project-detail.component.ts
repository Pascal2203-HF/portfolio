...
@Component({
  ...
  template: `
    <section class="section" *ngIf="project">
      <div class="container">
        <a routerLink="/projects" class="text-decoration-none">&larr; Retour aux projets</a>
        <h2 class="mt-3">{{ project.title }}</h2>
        <p class="opacity-50">{{ project.role }} · {{ project.realization_date }}</p>

        <img *ngIf="project.cover_image" [src]="img(project.cover_image)" class="img-fluid rounded-4 my-4 shadow" style="max-height:420px;width:100%;object-fit:cover;">
        ...
        <div class="row g-3 mt-4" *ngIf="galleryUrls().length">
          ...
          <div class="col-md-4" *ngFor="let g of galleryUrls()">
